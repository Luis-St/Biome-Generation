...
import net.luis.biome.util.random.LegacyRandomSource;
			...
			if (k < 1) {
				...
				for (int i1 = j + 1; i1 < k; ++i1) {
					if (i1 >= 0 && octaves.contains(l - i1)) {
						this.noiseLevels[i1] = new SimplexNoise(source);
					} else {
	...
	public double getValue(double x, double z, boolean simplex) {
		...
		for (SimplexNoise simplexnoise : this.noiseLevels) {
			if (simplexnoise != null) {
				d0 += simplexnoise.getValue(x * d1 + (simplex ? simplexnoise.xo : 0.0D), z * d1 + (simplex ? simplexnoise.yo : 0.0D)) * d2;
			}
