...
public class SimplexNoise implements Noise {
	...
	public SimplexNoise(RandomSource source) {
	...
	protected static double dot(int[] is, double x, double y, double z) {
		return (double) is[0] * x + (double) is[1] * y + (double) is[2] * z;
	}

	private double getCornerNoise3D(int i, double x, double y, double z, double d) {
		double d1 = d - x * x - y * y - z * z;
		double d0;
		if (d1 < 0.0D) {
			d0 = 0.0D;
		} else {
