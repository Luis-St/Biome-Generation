...
public class ImageRun {
	...
	public static void main(String[] args) throws IOException {
		...
		Thread temperature = new Thread(() -> {
			try {
				ImageIO.write(temperature(size), "png", new File(System.getProperty("user.home") + "/Desktop/temperature.png"));
	...
	private static BufferedImage temperature(int size) {
	...
	private static BufferedImage humidity(int size) {
		BufferedImage image = new BufferedImage(size, size, BufferedImage.TYPE_INT_ARGB);
		Graphics2D graphics = image.createGraphics();
		for (int x = 0; x <= size; x++) {
			for (int z = 0; z < size; z++) {
	...
	private static BufferedImage continentalness(int size) {
		BufferedImage image = new BufferedImage(size, size, BufferedImage.TYPE_INT_ARGB);
		Graphics2D graphics = image.createGraphics();
		for (int x = 0; x <= size; x++) {
			for (int z = 0; z < size; z++) {
	...
	private static BufferedImage erosion(int size) {
		BufferedImage image = new BufferedImage(size, size, BufferedImage.TYPE_INT_ARGB);
		Graphics2D graphics = image.createGraphics();
		for (int x = 0; x <= size; x++) {
			for (int z = 0; z < size; z++) {
